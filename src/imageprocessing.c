...
imagem abrir_imagem_process(char *nome_do_arquivo) {
  ...
  I.width = x;
  I.height = y;
  I.r = mmap(NULL,sizeof(float) * x * y, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS,0,0);
  I.g = mmap(NULL,sizeof(float) * x * y, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS,0,0);
  I.b = mmap(NULL,sizeof(float) * x * y, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS,0,0);
...
void liberar_imagem_process(imagem *I) {
  munmap(I->r, sizeof(imagem));
  munmap(I->g, sizeof(imagem));
  munmap(I->b, sizeof(imagem));
}
